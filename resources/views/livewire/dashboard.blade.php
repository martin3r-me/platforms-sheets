<x-ui-page>
    ...
    <x-ui-page-container>
        <div class="space-y-6">
            ...
            <x-ui-panel title="Ordner" subtitle="Ordner in diesem Team">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                    @foreach($folders as $folder)
                    <div class="p-4 rounded-lg border border-[var(--ui-border)]/60 bg-[var(--ui-muted-5)] hover:border-[var(--ui-primary)]/40 transition-colors">
            ...
            <x-ui-panel title="Spreadsheets" subtitle="Spreadsheets ohne Ordner">
                <div class="divide-y divide-[var(--ui-border)]/40">
                    ...
                    <a href="{{ route('sheets.spreadsheet.show', $spreadsheet) }}" wire:navigate
                       class="d-flex items-center gap-3 p-4 hover:bg-[var(--ui-muted-5)] transition-colors">
                        @svg('heroicon-o-table-cells', 'w-6 h-6 text-[var(--ui-primary)]')
                        <div class="flex-grow-1">
                            <div class="font-medium text-[var(--ui-secondary)]">{{ $spreadsheet->name }}</div>
                            @if($spreadsheet->description)
                            <div class="text-xs text-[var(--ui-muted)]">{{ $spreadsheet->description }}</div>
                            @endif
                        </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div class="p-3 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            <div class="text-xs text-[var(--ui-muted)]">Spreadsheets</div>
                            <div class="text-lg font-bold text-[var(--ui-secondary)]">{{ $stats['spreadsheets'] }}</div>
                        </div>
                        <div class="p-3 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            <div class="text-xs text-[var(--ui-muted)]">Zellen belegt</div>
                            <div class="text-lg font-bold text-[var(--ui-secondary)]">{{ number_format($stats['cells']) }}</div>
                        </div>
                    </div>
                </div>
            </div>
